...
f:\Omega\to\mathbb{R}^n$, se denomina \resa{ecuación autónoma}. De
...
\resa{punto de equilibrio} del sistema \eqref{ecuaaut}. El teorema
...
representación la llamaremos \resa{retrato de fases}
...
\resa{cualitativamente equivalentes}.
...
\resa{trayectorias} u \emph{órbitas}\index{orbitas@órbitas} de
...
similares, en los sistemas lineales, a \resa{centros}  cuando $n$
es par y a \resa{sillas} cuando $n$ es impar.
\end{ejemplo}
...
llaman la \resa{variedad estable} y la \resa{variedad inestable},
...
El concepto de \resa{flujo} es una manera nueva de mirar las
...
un \resa{grupo uniparamétrico}.

\section{Conjuntos invariantes}

El siguiente concepto de \resa{conjunto invariante} desempeña un
rol importante.
...
podemos definir \resa{conjunto invariante hacia el futuro}
...
\resa{aislado} de $f$, es decir $f(x^0)=0$ y existe una bola
...
cuando $t\to\infty$. Diremos que $x(t)$ \resa{entra} en el
...
este ejemplo vamos a ilustrar el método de las \resa{isoclinas}.
