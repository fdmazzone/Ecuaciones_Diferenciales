...
\onslide<+-> Supongamos que tenemos un grupo de Lie de simetrías $(\hat{x},\hat{y})$ de la ecuación
\boxedeq{y'=f(x,y)}{eq:eq_princi}

\onslide<+-> Supongamos  que las simetrías son no triviales, es decir no dejan invariante las soluciones. Según \eqref{eq:sol_inv} debemos tener
 \[\eta(x,y)\not\equiv f(x,y)\xi(x,y)\]
La razón de esta condición se verá más adelante.
