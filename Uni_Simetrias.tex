...
La forma de la ecuación sufiere el  \href{http://es.wikipedia.org/wiki/Ansatz}{ansatz}
...
\begin{frame}{Simetrías resuelven ecuaciones}
...
El conjunto de puntos donde $y=0$ es invariante. La ecuación característica reducida es.
...
No obstante  $(\xi,\eta)$ determinan las simetrías a traves de las ecuaciones
...
\nl \textbf{Ejemplo:} Estas ecuaciones pueden ser dificiles de resolver. Pero en algunos casos sencillos puede ser fácil. Supongamos dado  $(\xi,\eta)=(0,y)$. Las ecuaciones implican $\hat{x}$ independiente de $\epsilon$. Luego $\hat{x}=A(x,y)$ es función de $x,y$ solo. Además para $\hat{y}$ tenemos $\hat{y}=B(x,y)e^{\epsilon}$. Las condiciones iniciales implican $A(x,y)=x$ y $B(x,y)=y$.  
