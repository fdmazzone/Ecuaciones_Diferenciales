...
\end{block}
\nl La primera expresión es más asimetría, entre las variables $x$ e $y$ una de ellas es independiente ($x$) y la otra ($y$) independiente. 
...
\nl Puede que usar la notación como forma diferencial sea más efectivo. Como $r$ y $\theta$ son funciones de $x$ e $y$, son 0-formas. Usando las reglas de la diferencial, hay que reemplazar
\boxedeq{
\begin{array}{ll}
...
Los cambios de variables de un conjunto de dos variables, digamos $x$ e $y$, son funciones $\varphi$, invertibles,  de clase $C^{\infty}$, donde $\varphi:\Omega_1\to\Omega_2$, con $\Omega_1,\Omega_2$ abiertos de $\rr^2$. Acostumbraremos escribir $(\hat{x},\hat{y})=\varphi(x,y)$ y diremos que $(\hat{x},\hat{y})$ son la variables nuevas e $(x,y)$ las viejas.
...
\item Si $(x,y)\in\rr^2$ escribimos $(\hat{x},\hat{y})=f(x,y,\lambda):=P_{\lambda}(x,y)$.
\end{enumerate}
...
\item<+-> Si $P_{\lambda}(x,y)=(\hat{x},\hat{y})$, entonces  $\hat{x}(x,y,\lambda)$ y $\hat{y}(x,y,\lambda)$ son  diferenciables respecto $(x,y)$ y se desarrollan en serie de potencias respecto a $\lambda$. Es decir para todo $\lambda_0\in\rr$ 
\[
\begin{array}{cc}
\hat{x}(x,y,\lambda)&=a_0(x,y)+a_1(x,y)(\lambda- \lambda_0)+\cdots\\
\hat{y}(x,y,\lambda)&=b_0(x,y)+b_1(x,y)(\lambda- \lambda_0)+\cdots\\
\end{array}
\]
...
\[y'=f(x,y).\]
Una transformación $P\in \mathscr{T}$ se denomina una simetría de la ecuación si el cambio de variables dado por $(\hat{x},\hat{y})=P(x,y)$ deja invariante  la ecuación. 
...
De acuerdo con \eqref{eq:subsder} se debe cumplir que 
 ...
 Luego 
\[\frac{\partial\hat{y}}{\partial y}=1\Rightarrow \boxed{\hat{y}=y+\lambda} \]
...
\[
P_{\lambda}(x,y)= \begin{pmatrix} \hat{x}\\ \hat{y}
...
\[
P^{-1}_{\lambda}(\hat{x},\hat{y})= \begin{pmatrix} x\\ y
\end{pmatrix}=\begin{pmatrix} \cos(\lambda) & \sen(\lambda)
\\ -sen(\lambda) & \cos(\lambda)
...
\boxedeq{\frac{d\hat{y}}{d\hat{x}}=\sage{simplify(Expr9)}.}{}
\end{frame}
