...
\begin{tabular}{c c c}
 \includegraphics[scale=.1]{mu_neg.png} & \includegraphics[scale=.1]{mu_pos.png}  &
 \includegraphics[scale=.1]{mu_cero.png}\\
 $\mu<0$ & $\mu>0$ & $\mu=0$\\
\end{tabular}
...
\begin{tabular}{m{2cm} m{8cm}}
 \lstI &\hspace{-5cm}\includegraphics[scale=.2]{sobreamortiguado.png}
\end{tabular}
...
\boxedeq{x_1(t)=c_1e^{-\mu t}+c_2te^{-\mu t}=e^{-\mu t}\{c_1+c_2t\}}{eq:sol_gen_crit}
 \lstI 
 \begin{center}
   \includegraphics[scale=.1]{crit_amortiguado.png}
 \end{center}
...
\boxedeq{x(t)=e^{-\mu t}\left\{ c_1\cos \nu t+c_2\sen \nu t    \right\}}{eq:sol_gen_sub}
 \lstI 
 \begin{center}
   \includegraphics[scale=.1]{subamortiguado.png}
 \end{center}
...
chica $\omega-\omega_0$. 
\lstI
\includegraphics[scale=.2]{batido.jpg}
...
Vamos a considerar una fuerza externa oscilatoria de frecuencia $\omega_0$ y amplitud $F_0$. Tenemos que resolver
\boxedeq{x''(t)+2\mu x'(t)+\omega^2 x(t)=F_0\cos(\omega_0 t).}{eq:ecua_2orden_nohom}
...
\defverbatim[colored]\lstI{ 
\begin{lstlisting}
sage: latex(sqrt(A**2+B**2).subs(SolAB[0]).simplify_full())
\frac{F_{0}}{\sqrt{\omega^{4} + \omega_{0}^{4} + 2 \, 
{\left(2 \, \mu^{2} - \omega^{2}\right)} \omega_{0}^{2}}}
\end{lstlisting}
 }
