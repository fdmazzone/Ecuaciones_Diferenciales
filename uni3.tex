...
\begin{frame}{ Vibraciones amortiguadas no forzadas ($c>0$, $F=0$) }
De ahora en más supondremos que el sistema parte del resposo $x'(0)=0$ y de una posición indeterminada $x_0$. 
\lstI
...
\defverbatim[colored]\lstI{ 
\begin{lstlisting}
sage: t,mu,omega,omega0,F0=var('t,mu,omega,omega0,F0')
sage: A,B=var('A,B')                   
sage: x=A*cos(omega0*t)+B*sin(omega0*t)
sage: eq=x.diff(t,2)+2*mu*x.diff(t)+omega^2*x-F0*cos(omega0*t)
sage: eq=eq.simplify_full()
sage: show(eq)
sage: eq.coefficient(cos(omega0*t))
B*mu*omega0 + (omega^2 - omega0^2)*A - F0
sage: show(eq.coefficient(cos(omega0*t)))
sage: show(eq.coefficient(sin(omega0*t)))
sage: eqL1=eq.coefficient(sin(omega0*t))
sage: eqL2=eq.coefficient(cos(omega0*t))
sage: show([eqL1,eqL2]) 
CoefSis=matrix([[eqL1.coefficient(A),eqL1.coefficient(B)],[eqL2.coefficient(A),eqL2.coefficient(B)]])
sage: show(CoefSis.det()) 
sage: SolAB=solve([eqL1,eqL2],[A,B],solution_dict=True)    
sage: show(SolAB)   
sage: show(x.subs(SolAB[0]))
sage: x=x.subs(SolAB[0][0])
...
sage: x0=x(alpha=0)
sage: sol=solve([x0(t=0)==0,x0.diff(t).subs(t=0)==0],rho,solution_dict=True) 
...
\begin{frame}{Un poco de mecánica celeste}
   Los dos ingredientes básicos para derivar la leyes de movimiento del planeta so la 
  ...
  A la constante de proporcionalidad la llamaremos, como es costumbre, $G$. La dirección de la fuerza gravitatoria es la de la recta que une los  dos astros y el sentido es tal que la fuerza atrae los cuerpos. Vale decir, la dirección y sentido de la 
  fuerza de gravedad sola sera la del versor $-\v{r}/r$, donde $r=|\v{r}|$. Luego se debe satisfacer que
...
Todos los cálculos que realizamos los pueden encontrar en el repositorio de \href{https://github.com/fdmazzone/Ecuaciones_Diferenciales}{GitHub} 
...
Primero declaramos las variables, los vectores $\v{u}_r$ y $\v{u}_{\theta}$ y el vector 
...
\]
Notar que esta ecuación entraña dos incognitas $r$ y $\theta$, pero $\dot{\theta}$ puede ser rreemplazado por $h/r^2$ por la segunda Ley de Kepler.  
...
\begin{frame}[fragile]{Un poco de mecánica celeste}
Conseguimos una ecuación no lineal para $r$. Ahora el truco mágico consiste en considerar la nueva variable dependiente $z=1/r$ y la nueva variable independiente 
$\theta$. 
\begin{sageblock}
  z=function('z',theta)
  r=1/z
...
Resulta
\[\sage{ed4}\]
La ecuación del oscilador armónico. Sabemos resolver este ecuación, y SAGE también!!
\begin{sageblock}
  s=var('s')
...
\begin{frame}[fragile]{Un poco de mecánica celeste}
\textbf{Ejercicio:} La ecuación \eqref{eq:orb_elipse} es la ecuación de una cónica con foco en el origen, excentricidad $e$.
