...
% \pgfpagesuselayout{4 on 1}[letterpaper,landscape,border shrink=2.5mm]
% }
\title[Ecuaciones lineales ] % (optional, nur bei langen Titeln nötig)
{%
...
\end{frame}
\section[Estructura]{Estructura del conjunto de soluciones}
...
\end{frame}
\section[EHCC]{Ecuaciones homogéneas con coeficientes constantes}
...
-p**2*v(x)/4 + q*v(x) + Derivative(v(x), x, x)
\end{lstlisting}
}
...
\section[ENH]{Ecuación no homogénea}
...
%\subsection[MCI]{Método coeficientes ideterminados}
...
\defverbatim[colored]\lstI{
\begin{lstlisting}
>>> x,p,q,a,b,A,B=var('x,p,q,a,b,A,B')
...
\onslide<+-> Hay que proponer como solución un polinomio, en primera instancia, del mismo grado.
...
\end{pmatrix}
\]
Queda un sistema triángular superior que se resuelve por sustitución ascendente. Esto siempre que $q\neq 0$. En caso contrario la matríz es singular y es posible que el sistema no tenga solución.
