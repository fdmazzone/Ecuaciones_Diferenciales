...
\begin{ejercicio}\label{ejer: 1} Demostrar que las siguientes aplicaciones inducen grupos de Lie uniparamétricos
\begin{enumerate}
...
\begin{ejercicio} Encontrar coordenadas canónicas para las simetrías de los incisos 
\ref{item:1_ejer1} y \ref{item:2_ejer1} del ejercicio \ref{ejer: 1}. Repetir este mismo cálculo pero usando \texttt{SymPy} (o \texttt{SAGE}) con los incisos \ref{item:1_ejer1}, \ref{item:2_ejer1} y \ref{item:4_ejer1}.
\end{ejercicio}
...
\begin{ejercicio}
Considere la ecuación $y'=-\frac{1}{xy+g(y)}$.
\begin{enumerate}
\item[i)] Plantear la Condición de Simetría Linealizada y encontrar los infinitesimales $\xi$ y $\eta$. Ayuda: Hacer el anzats $\xi=\xi(y)$, $\eta\equiv 0$.
\item[ii)] Encontrar las coordenadas canónicas y plantear la ecuación en las mismas.
\item[iii)] Resolver la ecuación en las coordenadas canónicas y concluir que la solución en coordenadas rectangulares está dada por la relación implícita $e^{\frac{y^{2}}{2}}x+\int g(y) e^{\frac{y^{2}}{2}}dy=C$.
\end{enumerate}
