...
\texttt{SAGE} implementa formas diferenciales 
\begin{lstlisting}
sage: r,theta=var('r,theta')
sage: U = CoordinatePatch((r,theta)) #Declaramos coordenadas de un espacio
sage: F = DifferentialForms(U) #Declaramos conjunto  formas direfenciales
sage: x= DifferentialForm(F, 0, r*cos(theta)) #Definimos 0-formas
sage: y= DifferentialForm(F, 0, r*sin(theta))
sage: w=(x^3+x*y^2-x+y)*y.diff()-(y^3+x^2*y-x-y)*x.diff() #Definimos la 1-forma
sage: w[0].simplify_full() # que tiene dr?
sage: w[1].simplify_full() #que tiene d\theta?
\end{lstlisting}
