...
\begin{sageblock}
X=Matrix([Eq1,Eq2])
A=jacobian(X, (x,y)).subs({x:0,y:0})
D=A.eigenvalues()
\end{sageblock}

Los autovalores son $\sage{D}$. No  se puede aplicar el Teorema Linearización, autovalores imaginarios puros. Vamos a usar la energía
...
\begin{sageblock}
DLambdaX=jacobian(Lambda,(x,y))*X.T
\end{sageblock}

\[\nabla\Lambda\cdot X=\sage{DLambdaX}\]
...
\begin{sageblock}
p=contour_plot(Lambda,(x,-2*pi,2*pi),(y,-3,3)\
,contours=srange(-1,5,.2),fill=False)
\end{sageblock}
...
Esfe=parametric_plot3d([sqrt(3.0)*cos(theta)*sin(phi), sqrt(3.0)\
*sin(theta)*sin(phi), sqrt(3.0)*cos(phi)], (theta,0,2*pi)\
, (phi,0,pi),opacity=0.4)
Gra+=Esfe
...
Calculemos la matríz de monodromía. Empezamos por encontrar el flujo 
\begin{sageblock}
