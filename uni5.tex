...
Nuestro obtjetivo es encontrar dos soluciones linealmente independientes de la ecuación de Bessel. Hemos encontrado una que corresponde a la solución de la ecuación de índices $m=p$, la otra surgirá de la solución $m=p$. Pero aún nos restaría elegir el valor de $a_0$. El criterio que utilizaremos para elegirlo será que nos simplifique lo más posible la expresión  \eqref{eq:sol_bessel_1}. Si $p$ fuese un entero positivo, eligiendo $a_0$ como $1/2^pp!$ la expresiòn quedaría
...
\begin{definicion} Para $p>0$ definimos la \emph{función Gamma} por 
\boxedeq{\Gamma(p):=\int_0^{\infty}t^{p-1}e^{-t}dt}{eq:gamma}
\end{definicion}

En otros cursos se demuestra que la integral impropia en la definición converge. Adeás se 
...
Ahora vamos a tomar la solución $m=-p$ de la ecuación indicial.
\begin{sageblock}
...
% \]
% Obtenemos la solución 
% \boxedeq{y(x)=\sum_{n=0}^{\infty}\frac{(-1)^na_0}{4^nn!(p+n)(p+n-1)\cdots (p+1)}x^{2n}}{}
% Más adelante veremos que cierta elección especial de $a_0$ no lleva a lo que denominaremos funciones de Bessel.

% En SAGE solo nos restaba hacer las sustituciones de los coeficientes para arribar a esta función.
% \begin{sageblock}
% Coef=[f.factor() for f in Coef_Sol_1[0]]
% y1=y.subs(f).subs(Sol_Ecua_Ind[1])
% \end{sageblock}
